...
def hangman(secret_word):
    ...
    * After each guess, you should display to the user the 
    ...
    '''
    # FILL IN YOUR CODE HERE AND DELETE "pass"
    #secret_word, letters_guessed, guesses_remaining, warnings_remaining
    ...
    def get_score(guesses_remaining, secret_word):
        ...
        count = 0
        for i in secret_word:
            if i not in letter_tracker:
                letter_tracker.append(i)
                count += 1
        return count*guesses_remaining
    ...
    while is_word_guessed(secret_word, letters_guessed) == False:
        ...
        # If invalid input (not a letter or number):
        if guess.isalpha() == False or guess.isdigit() == True:
    ...
    if guesses_remaining > 0:
        print('-------------\nCongratulations, you won!\nYour total score for this game is: ' + str(get_score(guesses_remaining, secret_word)))
...
def match_with_gaps(my_word, other_word):
    ...
    if len(my_word) == len(other_word):
        my_word = dict(enumerate(my_word))
        other_word = dict(enumerate(other_word))
        for key in other_word.keys():
            if my_word[key] != '_':
                if my_word[key] == other_word[key]:
                    used_letters.append(my_word[key])
                else:
                    return False
        for key in other_word.keys():
            if my_word[key] == '_':
                if other_word[key] in used_letters:
                    return False
        return True 
    else:
        return False
...
def hangman_with_hints(secret_word):
    ...
    * If the guess is the symbol *, print out all words in wordlist that
    ...
    while is_word_guessed(secret_word, letters_guessed) == False:
        ...
        # If invalid input (not a letter or number):
        if (guess.isalpha() == False or guess.isdigit() == True) and guess != '*':
            warnings_remaining -= 1
            if warnings_remaining >= 0:
                print('Oops! That is not a valid letter. You have ' + str(warnings_remaining) + ' warnings left.\n' + get_guessed_word(secret_word, letters_guessed))
            else:
